...
def analyze_call_logs(backup_dir, os_version, xml_dir, config):
    ...
    ## Table Design
    #### [['_id', 'INTEGER'], ['number', 'TEXT'], ['date', 'INTEGER'],
    #     ['duration', 'INTEGER'], ['type', 'INTEGER'], ['new', 'INTEGER'],
    #     ['name', 'TEXT'], ['numbertype', 'INTEGER'],
    #     ['numberlabel', 'TEXT']]
        ...
        try:
            ...
            for i in range(1, len(result_list[CALL_LOG_TABLE_NUMBER])):
                ...
                call_log_list.append([
                    str(result_list[CALL_LOG_TABLE_NUMBER][i]
                        [CALL_LOG_ENTRIES_LIST[0]]),
                    str(result_list[CALL_LOG_TABLE_NUMBER][i]
                        [CALL_LOG_ENTRIES_LIST[1]]),
                    time.strftime("%a, %d %B %Y %H:%M:%S",
                                  time.gmtime(result_list
                                  [CALL_LOG_TABLE_NUMBER][i]
                                  [CALL_LOG_ENTRIES_LIST[2]] / 1000.0)),
                    str(result_list[CALL_LOG_TABLE_NUMBER][i]
                         [CALL_LOG_ENTRIES_LIST[3]]),
                    str(result_list[CALL_LOG_TABLE_NUMBER][i]
                       [CALL_LOG_ENTRIES_LIST[4]]), string_replace(
                       str(result_list[15][i][CALL_LOG_ENTRIES_LIST[5]]))])
        except:
            _adel_log.log("analyzeDBs:    ----> it seems that there are no\
                          call logs or that the database has an unsupported\
                          encoding!", 1)
        _xmlParser.call_log_to_xml(xml_dir, call_log_list)
    else:
        _adel_log.log("analyzeDBs:    ----> database file " + 
                      db_file_call_logs.split("/")[2] + " missing !!", 1)
...
def analyze_sms_mss(backup_dir, os_version, xml_dir, config):
    ...
    ## Table Design
    #### [['_id', 'INTEGER'], ['thread_id', 'INTEGER'], ['address', 'TEXT'], ['person', 'INTEGER'], ['date', 'INTEGER'], ['protocol', 'INTEGER'], ['read', 'INTEGER'], ['status', 'INTEGER'], ['type', 'INTEGER'], ['reply_path_present', 'INTEGER'], ['subject', 'TEXT'], ['body', 'TEXT'], ['service_center', 'TEXT'], ['locked', 'INTEGER'], ['error_code', 'INTEGER'], ['seen', 'INTEGER']]
...
def analyze_calendar(backup_dir, os_version, xml_dir, config):
    ...
    ## Table Design
    #### [['_id', 'INTEGER'], ['_sync_account', 'TEXT'], ['_sync_account_type', 'TEXT'], ['_sync_id', 'TEXT'], ['_sync_version', 'TEXT'], ['_sync_time', 'TEXT'], ['_sync_local_id', 'INTEGER'], ['_sync_dirty', 'INTEGER'], ['_sync_mark', 'INTEGER'], ['url', 'TEXT'], ['name', 'TEXT'], ['displayName', 'TEXT'], ['hidden', 'INTEGER'], ['color', 'INTEGER'], ['access_level', 'INTEGER'], ['selected', 'INTEGER'], ['sync_events', 'INTEGER'], ['location', 'TEXT'], ['timezone', 'TEXT'], ['ownerAccount', 'TEXT'], ['organizerCanRespond', 'INTEGER']]
    ## Tuple Definition -> the integers in this tuple represent the corresponding columns of the database table of the above table design
    ...
    ## Table Design
    #### [['_id', 'INTEGER'], ['_sync_account', 'TEXT'], ['_sync_account_type', 'TEXT'], ['_sync_id', 'TEXT'], ['_sync_version', 'TEXT'], ['_sync_time', 'TEXT'], ['_sync_local_id', 'INTEGER'], ['_sync_dirty', 'INTEGER'], ['_sync_mark', 'INTEGER'], ['calendar_id', 'INTEGER'], ['htmlUri', 'TEXT'], ['title', 'TEXT'], ['eventLocation', 'TEXT'], ['description', 'TEXT'], ['eventStatus', 'INTEGER'], ['selfAttendeeStatus', 'INTEGER'], ['commentsUri', 'TEXT'], ['dtstart', 'INTEGER'], ['dtend', 'INTEGER'], ['eventTimezone', 'TEXT'], ['duration', 'TEXT'], ['allDay', 'INTEGER'], ['visibility', 'INTEGER'], ['transparency', 'INTEGER'], ['hasAlarm', 'INTEGER'], ['hasExtendedProperties', 'INTEGER'], ['rrule', 'TEXT'], ['rdate', 'TEXT'], ['exrule', 'TEXT'], ['exdate', 'TEXT'], ['originalEvent', 'TEXT'], ['originalInstanceTime', 'INTEGER'], ['originalAllDay', 'INTEGER'], ['lastDate', 'INTEGER'], ['hasAttendeeData', 'INTEGER'], ['guestsCanModify', 'INTEGER'], ['guestsCanInviteOthers', 'INTEGER'], ['guestsCanSeeGuests', 'INTEGER'], ['organizer', 'STRING'], ['deleted', 'INTEGER'], ['dtstart2', 'INTEGER'], ['dtend2', 'INTEGER'], ['eventTimezone2', 'TEXT'], ['syncAdapterData', 'TEXT']]
    ## Tuple Definition -> the integers in this tuple represent the corresponding columns of the database table of the above table design
...
def analyze_contacts(backup_dir, os_version, xml_dir, config):
    ...
    ## Table Design
    #### [['_id', 'INTEGER'], ['name_raw_contact_id', 'INTEGER'], ['photo_id', 'INTEGER'], ['custom_ringtone', 'TEXT'], ['send_to_voicemail', 'INTEGER'], ['times_contacted', 'INTEGER'], ['last_time_contacted', 'INTEGER'], ['starred', 'INTEGER'], ['in_visible_group', 'INTEGER'], ['has_phone_number', 'INTEGER'], ['lookup', 'TEXT'], ['status_update_id', 'INTEGER'], ['single_is_restricted', 'INTEGER']]
    ## Tuple Definition -> the integers in this tuple represent the corresponding columns of the database table of the above table design
    ...
    ## Table Design
    #### [['_id', 'INTEGER'], ['is_restricted', 'INTEGER'], ['account_name', 'STRING'], ['account_type', 'STRING'], ['sourceid', 'TEXT'], ['version', 'INTEGER'], ['dirty', 'INTEGER'], ['deleted', 'INTEGER'], ['contact_id', 'INTEGER'], ['aggregation_mode', 'INTEGER'], ['aggregation_needed', 'INTEGER'], ['custom_ringtone', 'TEXT'], ['send_to_voicemail', 'INTEGER'], ['times_contacted', 'INTEGER'], ['last_time_contacted', 'INTEGER'], ['starred', 'INTEGER'], ['display_name', 'TEXT'], ['display_name_alt', 'TEXT'], ['display_name_source', 'INTEGER'], ['phonetic_name', 'TEXT'], ['phonetic_name_style', 'TEXT'], ['sort_key', 'TEXT'], ['sort_key_alt', 'TEXT'], ['name_verified', 'INTEGER'], ['contact_in_visible_group', 'INTEGER'], ['sync1', 'TEXT'], ['sync2', 'TEXT'], ['sync3', 'TEXT'], ['sync4', 'TEXT']]
    ## Tuple Definition -> the integers in this tuple represent the corresponding columns of the database table of the above table design
    ...
    ## Table Design
    #### [['_id', 'INTEGER'], ['package_id', 'INTEGER'], ['mimetype_id', 'INTEGER'], ['raw_contact_id', 'INTEGER'], ['is_primary', 'INTEGER'], ['is_super_primary', 'INTEGER'], ['data_version', 'INTEGER'], ['data1', 'TEXT'], ['data2', 'TEXT'], ['data3', 'TEXT'], ['data4', 'TEXT'], ['data5', 'TEXT'], ['data6', 'TEXT'], ['data7', 'TEXT'], ['data8', 'TEXT'], ['data9', 'TEXT'], ['data10', 'TEXT'], ['data11', 'TEXT'], ['data12', 'TEXT'], ['data13', 'TEXT'], ['data14', 'TEXT'], ['data15', 'TEXT'], ['data_sync1', 'TEXT'], ['data_sync2', 'TEXT'], ['data_sync3', 'TEXT'], ['data_sync4', 'TEXT']]
    ## Tuple Definition -> the integers in this tuple represent the corresponding columns of the database table of the above table design
